									...
									<td bgcolor="#4D595D" colspan="3" valign="top">
                						...
                						<table style="margin:20px 0px 0px 0px;" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
                						    ...
                						    <tr id="last_act_tr">
                						        <th>上次运行</th>
                						        <td>
                						            <% dbus_get_def("aliddns6_last_act", "--"); %>
                						        </td>
                						    </tr>
                						    <tr id="ak_tr">
                						        <th>app key</th>
                						        <td>
                						            <input type="text" id="aliddns6_ak" value="<% dbus_get_def("aliddns6_ak", ""); %>" class="input_ss_table">
                						        </td>
                						    </tr>
                						    <tr id="sk_tr">
                						        <th>app secret</th>
                						        <td><input type="password" id="aliddns6_sk" value="<% dbus_get_def("aliddns6_sk", ""); %>" class="input_ss_table"></td>
                						    </tr>
                						    <tr id="interval_tr">
                						        <th>检查周期</th>
                						        <td><input type="text" style="width: 2.5em" id="aliddns6_interval" value="<% dbus_get_def("aliddns6_interval", "120"); %>" class="input_ss_table">s</td>
                						    </tr>
                						    <tr id="name_tr">
                						        <th>域名</th>
                						        <td>
                						            <input type="text" style="width: 4em" id="aliddns6_name" placeholder="子域名" value="<% dbus_get_def("aliddns6_name", "home"); %>" class="input_ss_table"
                						            >.<input type="text"  id="aliddns6_domain" placeholder="主域名" value="<% dbus_get_def("aliddns6_domain", "example.com"); %>" class="input_ss_table">
                						        </td>
                						    </tr>
                						    <tr id="dns_tr">
                						        <th title="查询域名当前IP时使用的DNS解析服务器，默认为阿里云DNS">DNS服务器(?)</th>
                						        <td><input id="aliddns6_dns" class="input_ss_table" value="<% dbus_get_def("aliddns6_dns", "223.5.5.5"); %>"></td>
                						    </tr>
                						    <tr id="curl_tr">
                						        <th title="可自行修改命令行，以获得正确的公网IP。如添加 '--interface vlan2' 以指定多播情况下的端口支持">获得IP命令(?)</th>
                						        <td><textarea id="aliddns6_curl" class="input_ss_table" style="width: 94%; height: 2.4em"><% dbus_get_def("aliddns6_curl", "curl -s whatismyip.akamai.com"); %></textarea></td>
                						    </tr>
                						    <tr id="ttl_tr">
                						        <th title="设置解析TTL，默认10分钟，免费版的范围是600-86400">TTL(?)</th>
                						        <td><input id="aliddns6_ttl" style="width: 4.5em" class="input_ss_table" value="<% dbus_get_def("aliddns6_ttl", "600"); %>">s (1~86400)</td>
                						    </tr>
                						</table>
                						<div class="apply_gen">
...
$(function () {
    show_menu(menu_hook);
    var enable = "<% dbus_get_def("aliddns6_enable", "0"); %>";
    ...
    $('.button_gen').click(function () {
        ...
        posting = true; // save
		var data = {
			aliddns6_enable: $('#switch').prop('checked') | 0,
			action_mode: ' Refresh ',
			current_page: 'Module_aliddns6.asp',
			next_page: 'Module_aliddns6.asp',
			SystemCmd: 'aliddns6_config.sh'
		};
		for(var i = 0; i< inputs.length; i++) {
			var key = 'aliddns6_' + inputs[i];
			data['aliddns6_' + inputs[i]] = $('#aliddns6_' + inputs[i]).val()
		}
        $.ajax({
            type: 'POST',
            url: 'applydb.cgi?p=aliddns6_',
            data: $.param(data)
        }).then(function () {
            posting = false;
            alert('saved');
...
function menu_hook(title, tab) {
	if(enable_ss == "1" && enable_soft == "1"){
		...
		tablink[17] = new Array("", "Module_aliddns6.asp");
	}else{
		tabtitle[16] = new Array("", "Aliddns");
		tablink[16] = new Array("", "Module_aliddns6.asp");
	}
}
